...
func Middleware(writer http.ResponseWriter, request *http.Request) {
	...
	if !strings.Contains(request.Header.Get("Cookie"), "__Proxy_v="+encryptedIP) {
		...
		case 1:
			writer.Header().Set("Set-Cookie", "_1__Proxy_v="+encryptedIP+"; SameSite=Lax; path=/; Secure")
		...
		case 3:
			...
			if !captchaExists {
				...
				if err := png.Encode(&captchaBuf, captchaImg); err != nil {
					SendResponse("Cloudflare Error: Failed to encode captcha: "+err.Error(), buffer, writer)
					return
				}
				if err := png.Encode(&maskBuf, maskImg); err != nil {
					SendResponse("Cloudflare Error: Failed to encode captchaMask: "+err.Error(), buffer, writer)
					return
				}
	...
	case "/_bProxy/credits":
		writer.Header().Set("Content-Type", "text/plain")
		SendResponse("Cloudflare; Lightweight http reverse-proxy https://github.com/41Baloo/Cloudflare. Protected by GNU GENERAL PUBLIC LICENSE Version 2, June 1991", buffer, writer)
		return
	}
